...
function App() {
  ...
  const handleFiles = async (files) => {
    ...
    if (uploadedImages.length + files.length > 8) {
      alert('Collage Maker supports a maximum of 8 images per collage. Please reduce file count.');
      return;
    }
    ...
    <div className="app-container">
      ...
      <header className="header glass-panel">
        <div className="header-title">
          ...
          <p>High-performance image processing suite</p>
        </div>

        {/* Multi-tool navigation tabs */}
        <nav className="nav-tabs">
          <button 
            className={`nav-tab ${activeTab === 'collage' ? 'active' : ''}`}
            onClick={() => setActiveTab('collage')}
          >
            🖼️ Collage Maker
          </button>
          <button 
            className={`nav-tab ${activeTab === 'compress' ? 'active' : ''}`}
            onClick={() => setActiveTab('compress')}
          >
            🗜️ Compress & Convert
          </button>
          <button 
            className={`nav-tab ${activeTab === 'resize' ? 'active' : ''}`}
      ...
      {activeTab === 'collage' && (
        <main className="main-content">
                ...
                {activeTemplate.slots.map((slot, index) => {
                  ...
                  return (
                    ...
                    >
                      {img ? (
                        <img 
                          src={`${BACKEND_URL}/uploads/${img.id}`} 
                          alt="Placed" 
                          style={{ borderRadius: `${borderRadius}px` }} 
                        />
                      ) : (
                        <div className="slot-placeholder" style={{ borderRadius: `${borderRadius}px` }}>
          ...
          <section className="sidebar glass-panel">
            ...
            <h2 style={{ marginTop: '8px' }}>3. Select Template</h2>
            <div className="control-group">
            ...
            <h2 style={{ marginTop: '8px' }}>4. Export</h2>
            <div className="control-group">
